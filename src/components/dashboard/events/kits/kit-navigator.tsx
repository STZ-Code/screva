...
export function KitNavigator({ slug }: KitNavigatorProps) {
	...
	return (
		<div className="flex h-fit xl:flex-row flex-col xl:gap-0 gap-6 relative xl:border-b border-zinc-700 justify-between">
			<TabNavigator.Root active={pathname} className="self-end">
				<TabNavigator.Control className="gap-4">
					<TabNavigator.Item
						...
						as={Link}
						className={cn(
							'font-medium text-zinc-400 xl:text-xl text-sm px-4 flex items-center gap-3 pb-4',
							{
					...
					<TabNavigator.Item
						...
						as={Link}
						className={cn(
							'font-medium text-zinc-400 flex-1 xl:text-xl text-sm px-4 flex items-center gap-3 pb-4',
							{
								'text-zinc-50': !isConfigPage,
							},
						)}
